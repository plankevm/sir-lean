...
issues a CALL whose child *terminates as a black box* (`hchild`: any `drive … = .ok
childRes`, no oracle), then `Runs` its suffix to a halt, produces exactly the
caller's halt result as the top-level `messageCall`. The single numeric side
condition is the fuel bound `hfuel` — pure gas arithmetic, **not** the conclusion.
The observable-level lift `messageCall_call_completedWith` adds success+cell
hypotheses to land the named `Outcome.completedWith`.
...
- **`messageCall_call_runs`** is the headline: the sound, program-agnostic CALL
  ...
  executes — the child is a black box, the caller's prefix/suffix are honest `Runs`
  traces, and the one numeric premise is the fuel bound. No hypothesis is
...
- `messageCall_call_runs`: the sound, program-agnostic CALL sequencing rule (black-
  box child, honest caller traces, one numeric fuel side condition).
