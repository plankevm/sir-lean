...
open Lir (AccPresent accMono_of_accounts_eq) in
...
theorem sstore_accMono (st : State) (key val : UInt256) (a : AccountAddress)
    (h : Lir.AccPresent a st.accounts) :
    Lir.AccPresent a (st.sstore key val).accounts := by
  unfold State.sstore
  ...
  | some acc =>
    simp only [hr]
    exact Lir.accounts_find?_insert_mono _ _ _ _ h
...
theorem tstore_accMono (st : State) (key val : UInt256) (a : AccountAddress)
    (h : Lir.AccPresent a st.accounts) :
    Lir.AccPresent a (st.tstore key val).accounts := by
  unfold State.tstore
  simp only [State.lookupAccount, Option.option]
  cases hr : st.accounts.find? st.executionEnv.address with
  | none => simpa only [hr] using h
  ...
  | some v =>
    ...
    | ok em =>
      ...
      | ok ec =>
        ...
        rw [continueWith_next h, ExecutionState.replaceStackAndIncrPC]
        rw [Lir.charge_pc hc, Lir.chargeMemExpansion_pc hm]
        rfl
...
theorem charge_sstore_next_accMono {cost : ℕ} {exec exec' : ExecutionState} {key newVal : UInt256}
  ...
  | ok ec =>
    ...
    · rw [replaceStackAndIncrPC_accounts]
      ...
      exact Lir.accMono_of_accounts_eq a hcacc hp
      ...
      | ok e2 =>
        ...
        · -- next (fallback) branch
          ...
          · show AccPresent a (resumeAfterCall _ _).exec.accounts
            rw [Lir.resumeAfterCall_accounts]
    ...
    | ok e1 =>
      ...
      | ok e2 =>
        ...
        · simp only [Except.ok.injEq, Signal.next.injEq] at h
          ...
          rw [replaceStackAndIncrPC_pc, Lir.charge_pc he2,
            Lir.charge_pc he1]
          rfl
        ...
        · cases hm : chargeMemExpansion exec io is with
          ...
          | ok em =>
            ...
            | ok ec =>
              ...
              rw [hpc', Lir.charge_pc hc, Lir.chargeMemExpansion_pc hm]
  · unfold systemOp at h
    simp only [bind, Except.bind] at h
    cases hr : requireStateMod exec with
    | error e => rw [hr] at h; simp at h
      ...
      · cases hm : chargeMemExpansion exec io is with
        ...
        | ok em =>
          ...
          | ok ec =>
            rw [hc] at h
            simp only [] at h
            rw [createArm_next_memory h, Lir.charge_memory hc,
              Lir.chargeMemExpansion_memory hm]
      ...
      · cases hm : chargeMemExpansion exec io is with
        ...
        | ok em =>
          ...
          | ok ec =>
            rw [hc] at h
            simp only [] at h
            rw [createArm_next_accounts h, (Lir.charge_accounts_env hc).1,
              (Lir.chargeMemExpansion_accounts_env hm).1]
      ...
      · cases hm : chargeMemExpansion exec io is with
        ...
        | ok em =>
          ...
          | ok ec =>
            ...
            rwa [Lir.charge_activeWords hc, Lir.chargeMemExpansion_activeWords hm] at hge
...
open Lir (AccPresent accMono_of_accounts_eq accounts_find?_insert_mono) in
...
theorem returnOrRevertOp_success_accMono {op : Operation.SystemOp} {exec e : ExecutionState}
  ...
  | some v =>
    ...
    | ok em =>
      rw [hm] at h; simp only [] at h
      obtain ⟨hmacc, _⟩ := Lir.chargeMemExpansion_accounts_env hm
      split at h
