...
theorem toBytes!_length (x : UInt256) : (toBytes! x).length = 32 := by
  unfold toBytes!
  exact zeroPadBytes_len (toBytes'_UInt256_le x.val.isLt)

def uInt256OfByteArray (arr : ByteArray) : UInt256 :=
  .ofNat <| fromBytes' arr.data.toList.reverse

theorem UInt256.ofNat_toNat (x : UInt256) : UInt256.ofNat x.toNat = x := by
  cases x with
  | mk val =>
    cases val with
    | mk n h =>
...
theorem fromBytes'_toBytes! (x : UInt256) : fromBytes' (toBytes! x) = x.toNat := by
  simp [toBytes!, UInt256.toNat, fromBytes'_zeroPadBytes_32_eq, fromBytes'_toBytes']
