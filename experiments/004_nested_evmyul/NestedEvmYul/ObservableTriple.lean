...
**STATUS (post-T4): this file is placeholder-free and foundation-grade throughout,
...
4. Lifts the ∀-fuel relational veneer to the observable
   (`ΘRuns_completedWith`). This direct bounded-offset projection predates the
   now-proved fuel-monotonicity keystone; `ΘRuns.runΘ_complete'` additionally
   provides offset-unbounded adequacy under the structural depth cap.
...
Refinement.lean) `completedWith` value `0` at EVERY cell. PROVED without placeholders —
...
PROVED without placeholders by instantiating the family at `f := seedFuel w - k`. This
bounded projection remains useful without a depth premise. The stronger
`ΘRuns.runΘ_complete'` removes the offset bound by using the proved
`FuelMono.Θ_fuel_mono_ok` / `_error` pair together with the seeded never-OOF
fact; see ThetaRuns.lean's historical post-mortem. -/
...
**proved without placeholders** on the T1 XLoop vocabulary: a caller whose code performs
...
* this direct `ΘRuns_completedWith` route supplies
  `n₁ + n₂ + k₁ + k₂ + 6 ≤ seedFuel w`; the keystone-backed alternative trades
  that numeric bound for the structural depth cap.
...
was a classified-hard placeholder with three recorded blockers — no X-decomposition,
...
* **(a) Relational veneer — cofinal core plus a proved keystone.** The
  forall-fuel pivot re-encoded the veneer offset-cofinally
  (`∃ k, ∀ f, Θ (k + f) … = .ok res`), making determinism pure instantiation.
  Under the pre-patch semantics the CREATE/CREATE2 absorption made the desired
  fuel-monotonicity statement false. After honest error propagation landed,
  `FuelMono.res_mono` proves the six-layer transport and restores single-point
  introduction plus offset-unbounded adequacy under the depth cap (see
  ThetaRuns.lean's historical post-mortem). This file retains a direct bounded
  projection because it needs no depth premise.
