...
The correctness file no longer contains prefix/canonical proof fragments. Its main spec is:

```lean
LoweringPreservesSemantics oracle sourceFuel evmFuel program initial
```
...
```lean
EVM.X evmFuel (EVM.D_J (Bytecode.lower program) 0)
...
7. Compose the general `LoweringPreservesSemantics` theorem by induction over `Program`.
