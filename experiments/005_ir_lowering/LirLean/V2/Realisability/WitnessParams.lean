...
* `beginCall_inr_noErase` — the **engine fact** behind the `hseams.noErase` seam,
  ...
  doubles as the machine-check of this fact (`Surface.lean`, `PrecompileAssumptions`
  docstring: "a failure there is diagnosed as a SEAM problem with the engine stub");
* `callsCodeOk` — a fuel-indexed **trace checker** for the `hseams.callsCode` seam: it
  replays the deterministic top-level chain (`stepFrame` steps, returning CALLs,
  returning CREATEs) and checks every issued `.needsCall`'s code source. Soundness
  (`callsCode_of_entryCheck`) turns ONE `Bool` evaluation into the
  `∀ fr', ReachableFrom params fr' → CallsCode fr'` universal — the chain is linear
  because `stepFrame`/`beginCall`/`drive`/the resumes are functions;
...
## Why the two leaves are not discharged here (`decide +kernel` infeasibility, measured)

Both leaves are decidable and TRUE — validated by native evaluation (compiled probe,
`runWithLog exParams (seedFuel 25000) = some log` with `log.cleanb = true`, 2 gas
reads / 1 sload / 1 call / rem 179; NOT citable as proof). Discharging them in-kernel
(`decide +kernel`; `native_decide` is banned repo-wide) was attempted and is
INFEASIBLE on the current engine: the v4.30 kernel's lazy whnf explodes on the CALL
descent's state duplication (`endCall`/`resumeAfterCall` embed the suspended parent
frame and the child result, each carrying the full account/substate terms, and the
kernel's pointer-keyed cache cannot share the copies). Measured gas-prefix ladder,
kernel wall-clock: 100 ↦ 1s, 3000 ↦ 2s, 10000 ↦ 2s, 23000 ↦ 5s, 24400 ↦ 5s,
24700 (dies AT the CALL charge, ~65 top-level steps incl. cold SLOAD + cold SSTORE)
↦ 5s; 24770 (the first value whose run RESUMES from the returning CALL) ↦ OOM-killed
(>30 GB, 15 min), 24800/25000 ↦ OOM-killed (~60 GB, 10–15 min, 96 GB machine). The
kernel evaluates the LOWERING itself fine (`(Lir.lower exProg).size = 562` in ~1s) —
the wall is specifically the interpreter's call-resume step. Identified follow-up
route (not attempted, multi-hour): segmented evaluation — serialize literal machine
configs every K steps (custom `ToExpr`/renderer for `Frame`/RBMap trees) and close
each K-step segment by `rfl` from a literal start, resetting the laziness; the
segment lemmas compose by a generic fuel-shift lemma.
-/
...
/-- Executable `Bool` twin of `RunLog.clean` (`Surface.lean`): the recorded top-level
...
/-- Fuel-indexed trace checker for the `CallsCode` seam: replay the deterministic
top-level chain (`stepFrame` steps, returning CALLs, returning CREATEs) from `fr` and
check every issued `.needsCall`'s code source. `false` on fuel exhaustion (sound:
the caller must supply enough fuel); edges the `Runs` relation cannot take (a
non-returning call/create child, a precompile-immediate `beginCall`) end the chain
with `true` — no `Runs`-reachable frame lies beyond them. -/
...
and is clean (`exCheck`), and the `CallsCode` trace check passes (`entryCallsCodeOk`)
...
`beginCall_inr_noErase` theorem with the checker soundness `callsCode_of_entryCheck`.
