...
theorem exProg_nonvacuity :
  ...
  -- (all `decide`/`rfl` on the concrete program), which we supply directly (same module).
  -- Axiom-clean once R11 + R12a's two decidable leaves land. No single-call premise —
