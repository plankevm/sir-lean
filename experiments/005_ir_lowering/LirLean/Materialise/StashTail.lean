...
The spilled-SLOAD stash is `materialise k ++ [SLOAD] ++ PUSH32 slot ++ MSTORE`: materialise the
key `k` (the **B1** `materialise_runs` prefix — a non-gas/non-sload `Expr`), `SLOAD` the cell at
`k`, then the core tail stashes the loaded value at `slot`. This composes three existing GREEN
pieces — `materialise_runs` (B1), `sim_sload` (the `Match` SLOAD brick), and `stash_tail_runs` —
with no new spine decode primitive.

The materialise-key endpoint `frk` and its `MatRuns` bundle are taken as *inputs* (the caller
runs `materialise_runs` and supplies them, exactly as `sim_assign_sload_lowered` does over
...
`MatRuns` guarantees `memBytes` equality unconditionally but only `activeWords`-nondecreasing). The
loaded value is tied to `w` through `selfStorage fr keyVal = w` (the `Corr`/`StorageAgree` storage
lens at the materialised key, threaded by `MatRuns.storage`). -/

/-- **The spilled-SLOAD def-site stash `materialise k ; SLOAD ; PUSH32 slot ; MSTORE`
(forward lemma, P-walk).** From the statement boundary `fr` (`stack = []`), given the
materialise-key endpoint `frk` (the `MatRuns` bundle for `.tmp k`, leaving the key value `keyVal`
...
theorem stash_tail_sload {defs : Tmp → Option Expr} {sloadChg : Tmp → ℕ} {fuel : Nat}
    (fr frk : Frame) (k : Tmp) (keyVal w : Word) (slot : Nat) (words' : UInt64)
    (hstk : fr.exec.stack = [])
    (hmrk : MatRuns defs sloadChg fuel (.tmp k) keyVal fr frk)
  ...
  · -- pc: (frk.pc + 1) + 34 = fr.pc + ((mat k).length + 35); frk.pc = fr.pc + (mat k).length.
    rw [hpc, hspc, hmrk.pc, UInt32.ofNat_add,
