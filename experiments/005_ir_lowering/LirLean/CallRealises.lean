...
The now-deleted `LirLean/Oracle.lean` discharged the *gas* stream's realisability: the abstract
...
never an axiom. This module is the **call** analogue: an abstract `Lir.CallStream` entry
(`Spec/Semantics.lean`) is *realised* by the frame-reference `evmCallOracle`
...
A `Lir.CallStream` entry is a `(World × Word)` — (post-call world, 0/1 success). The realised
...
/-! ## Step 6 — the realised `Lir.CreateStream` entry from the frame-reference `evmCreateOracle`

The CREATE twin of `evmV2CallEntry`/`callRealises_bridge`. A `Lir.CreateStream` entry is a
