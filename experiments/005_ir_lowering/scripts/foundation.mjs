...
- Work ONLY in ${WT} (git branch 'foundation'). Warm APFS CoW .lake: \`cd ${WT} && lake build\` (default LirLean, sorry-free spine) and \`lake build Nightly\` (the WIP conformance lib = LirLean/V2/RealisabilitySpec.lean) are INCREMENTAL (minutes; run in background if >8min). Never \`lake update\`, never delete .lake, never push.
...
TASK: redesign \`CallOracle\` as a consumed stream of recorded call-results (e.g. \`List (World × Word)\` consumed head-first, threaded alongside the gas Trace — pick the cleanest representation consistent with the existing gas threading). Update: EvalStmt.call (consume the head call-result instead of applying a function), RunStmts / RunFrom / IRRun (thread the call-stream like T), the determinism proofs (V2/Law.lean), and \`realisedCall\` / \`callOracleOf\` (produce the FULL stream from log.calls, not just the head). Re-prove ALL breakage across V2/Call.lean, V2/CallRealises.lean, DriveSim.lean, the sim lemmas, and RealisabilitySpec.lean.
