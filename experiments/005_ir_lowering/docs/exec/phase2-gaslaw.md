...
- `LirLean/Mono.lean` (70 decls) — the two-read gas-monotonicity milestone: the guard
  bytecode/frames (`guard_*`, `gf*`, `q0..q6`), `gReads_*`, `toNat_ofUInt64`,
  `LoweredRunHasObsMono`, `lower_preserves_obs_mono`.
- `LirLean/Oracle.lean` (9 decls) — the law-first gas-oracle interface:
...
- `LirLean/HonestGasTie.lean` (8 decls) — the retired-universal regression witnesses:
...
- `LirLean/RunLog.lean` — the whole gas-monotonicity section (pre-edit lines
  ...
  about the law. `import LirLean.Oracle` (the one dead import) dropped.
- `LirLean/Law.lean` — narrowed to determinism: deleted §1–§2
...
`Lir.gasReadOf` and `Lir.FramesRun` are consumed **in code** by
`TieDischarge.lean` (`gasRecord_eq_gasReadOf`, `gasReadOf_gasFrame_eq_obs`,
...
`LirLean/RunLog.lean` — the unique Track-B-owned file inside TieDischarge's import
cone — under a `-- RELOCATED from Oracle.lean (Phase 2)` note.
...
## DEVIATION from the plan: root gains `import LirLean.TieDischarge`

The plan's Step 3 deleted the HonestGasTie import block without replacement. But
`HonestGasTie.lean:1` (`import LirLean.TieDischarge`) was the **only** path from the
...
Fix: one direct `import LirLean.TieDischarge` in the root (commented), restoring the
...
```
'Lir.lower_conforms_cyclic_assembled' depends on axioms: [propext, Classical.choice, Quot.sound]
'Lir.lower_conforms_cyclic_tiefree' depends on axioms: [propext, Classical.choice, Quot.sound]
'Lir.lower_conforms_wf' depends on axioms: [propext, Classical.choice, Quot.sound]
'Lir.callPreservesSelf_modGuards' depends on axioms: [propext, Classical.choice, Quot.sound]
'Lir.driveLog_drive' depends on axioms: [propext, Classical.choice, Quot.sound]
'Lir.runWithLog_drive' depends on axioms: [propext, Classical.choice, Quot.sound]
'Lir.runWithLog_messageCall' depends on axioms: [propext, Classical.choice, Quot.sound]
'Lir.observe' depends on axioms: [propext, Classical.choice, Quot.sound]
'Lir.gasReadOf_gasFrame_eq_obs' depends on axioms: [propext, Classical.choice, Quot.sound]
'Lir.FramesRun.snoc' depends on axioms: [propext, Classical.choice, Quot.sound]
```
...
- Residual-reference grep over the deleted decl names: comment-only hits
  ...
  docstring).
- Untouched per constraints: `TieDischarge.lean`, the `DriveCorrPlus` accumulator
...
- `TieDischarge.lean:7, 57–68, 137–144, 204–210` — `Oracle.GasRealises` /
  `Oracle.lean` mentions (file untouchable this wave).
- `CallRealises.lean:9, 32, 88` — `GasRealises.monotoneGas`.
- `MaterialiseRuns.lean:552` — "…`Lir.GasRealises` (`Oracle.lean`)…".
- `MaterialiseRuns.lean:530` — mentions `HonestGasTie.lean` +
