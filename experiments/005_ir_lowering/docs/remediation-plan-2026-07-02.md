...
| B | `Match.lean` | delete 5 superseded per-construct sims (`sim_jump`, `sim_branch`, `sim_pop`, `applyCall_reflects_lowered`, `bindCallResult_reflects_lowered`). Keep `call_reflects_lowered` (audit §9). |
...
Unify first: prove `stepFrame_next_execEnvAddr` once, derive the ~920-line `SelfAt`/`_next_self` walk as the `a:=self` corollary of `_next_accMono` (saves ~1000 lines). Then move the pure-`Evm` engine layer (dispatch walks, `drive_accounts_find_mono`, begin/end/checkpoint/resume facts, RBMap primitives — per-decl, not a contiguous cut) into a new `003_bytecode_layer/BytecodeLayer/Hoare/AccountsMonotone.lean`; re-wire exp005 imports. Build both experiments green.

## Open decisions for the lead
1. **`HonestGasTie.lean`:** keep the SLOAD retired-universal regression witnesses (tiny standalone file) or delete with the rest of the gas apparatus? (Gas-law ones die regardless.)
...
## Execution log
- 2026-07-02: audit + map committed (ef9dbab). Plan + gas-decision written. Phase 1 launched.
