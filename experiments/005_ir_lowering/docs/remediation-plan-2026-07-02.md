...
| D | `LirLean.lean` + `lakefile.lean` | remove `WorkedCall.lean`(1752), `Decode.lean`(199), `WorkedCallParity.lean`(211) from the live module tree (already build-excluded; grep-confirm not imported); update NOTE comments |
...
- 2026-07-02: **Phase 1 DONE + green + axiom-clean** (3aaed43). 1164 jobs; headline `lower_conforms_cyclic_assembled` on `[propext, Classical.choice, Quot.sound]`; zero sorry/native_decide/errors/warnings. Removed ~742 lines net of dead scaffolding across 8 files; `TieDischarge.lean` 5027→4507; 3 leaf examples moved off-cone and were later removed from the tree; salvage bricks retained-with-label; overclaiming docstrings fixed. Living docs revitalized. `main` working tree left clean (repo-root doc edits relocated onto this branch).
