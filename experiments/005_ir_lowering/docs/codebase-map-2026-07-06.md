...
|---|---|---|---|
| `Spec/` | trusted spec surface | L1 (IR.lean), L2 (Semantics.lean), L3 (Lowering.lean), L7 (Recorder.lean); Conformance.lean is a **prose stub** (Conformance.lean:1-24, vacuous headline deleted 2026-07-03); Seams.lean is a **stale debt register** keyed to the deleted headline (Seams.lean:14-15) | Cannot state the headline from Spec/ alone — the L9 statement vocabulary is missing (see §4.1) |
...
| `Assembly/` | (name suggests an assembler) | L6: tie-unit definitions, decode-bundle discharge wrappers (LowerDecode), whole-CFG `sim_cfg`, static Acyclic discharge | Nothing assembles bytes (emission is Spec/Lowering); "Conformance/" or "CfgSim/" honest |
...
definitionally trusted), the L1–L3 spec files, and the L9 statement vocabulary currently stranded
in `V2/Realisability/Surface.lean`.

The **proof tower** runs: geometry (L4) turns global decode obligations into local byte facts;
the value channel (L5a) proves `materialiseExpr` bytes reconstruct `evalExpr` values on the EVM
...
1. **The trusted surface is split across the WIP lib.** `Spec/Conformance.lean` is a stub; the
   flagship `lower_conforms` (RealisabilitySpec.lean:206) and its entire hypothesis vocabulary
   (`Conforms`, `entryState`, `RunLog.clean`, `WellLowered`, `PrecompileAssumptions`,
   `NoGasReads`, `RunFromAll`) live in the non-default WIP lib — all sorry-free
   (RealisabilitySpec.lean:12: "All defs/structures here are REAL"), so nothing blocks hoisting.
...
def materialiseExpr (defs : Tmp → Option Expr) : Nat → Expr → List UInt8  -- :142
def defsOf (prog : Program) : Tmp → Option Expr                -- :262 (oracle temps ↦ .slot (slotOf t))
def allocate (prog : Program) : Alloc                          -- :291
def emit (a : Alloc) (prog : Program) : List UInt8             -- :327
def lower (prog : Program) : ByteArray := encode (emit (allocate prog) prog)  -- :339
```
...
theorem wellFormedLowered_of_acyclic (h : AcyclicWellFormed prog) : WellFormedLowered prog  -- :204
```
...
theorem lower_conforms {prog params log acc}                       -- RealisabilitySpec.lean:206 (R11, sorry)
    ...
    (hgas : Gjumpdest ≤ params.gas.toNat) (hwl : WellLowered prog)
...
structure WellLowered (prog : Program) : Prop                      -- Surface.lean:401
  -- wf : WellFormedLowered; defs : RunDefinableG; defsCons; entry0; closed : ClosedCFG;
  -- stack : StackRoomOK; gasBound; slotAddr; retEpilogueBound; noSlotSource
structure PrecompileAssumptions (prog params) : Prop where          -- Surface.lean:474
...
1. **Flagship statement vocabulary → `Spec/`** (highest value). `Conforms` (Surface.lean:63),
   `entryState` (:34), `RunLog.clean` (:51), `WellLowered` + components ClosedCFG/StackRoomOK/
   StmtDefinableG/RunDefinableG/DefsConsistent (:73-451), `PrecompileAssumptions` +
   `ReachableFrom` (:457-484), `NoGasReads` (:486), `RunFromLeft`/`RunFromAll` + adequacy
   (:894-982), plus `evmV2CallEntry`/`evmV2CreateEntry` (CallRealises.lean:59/:117) and
   `WellFormedLowered` (LowerConforms.lean:143). All appear in the flagship's *statement*; all
   are sorry-free (RealisabilitySpec.lean:12); today a reviewer of the nominal trusted surface
   cannot state what the project claims (Conformance.lean is a stub). Natural homes: a real
   `Spec/Conformance.lean` (Conforms/entryState/clean/WellLowered/PrecompileAssumptions),
   `Spec/Semantics.lean` (the RunFromLeft mirror, next to RunFrom — the anti-vacuity
   strengthening currently lives *outside* Spec while the weak RunFrom lives inside),
   `Spec/Seams.lean` (PrecompileAssumptions/NoGasReads, re-keying the register — see #7).
...
5. **`Decode/LoweringLemmas.lean` → Spec-companion or `Materialise/`.** Zero geometry content —
   allocation-faithfulness (`allocate_toDefs` :105) and spill-routing exhaustiveness
   (`defsOf_ne_gas`/`_ne_sload`); parked in Decode/ by convenience, pollutes the folder charter.
...
7. **`Spec/Seams.lean` → re-key or retire to docs.** Keyed to the *deleted* conditional headline
   (Seams.lean:14-15 vs Conformance.lean:8-14); under the current flagship, SelfPresent and
   CleanHaltsNonException are derived, not supplied, and the live seam is PrecompileAssumptions —
   whose `noErase` field (Surface.lean:476-477) is a verbatim textual duplicate of Seams'
   `PrecompilesPreservePresence` (Seams.lean:59-62) with **no definitional link** (drift-prone).
...
**5.1 The trusted spec surface cannot state the claim.** Spec/Conformance.lean:1-24 is a prose
stub; the flagship and its hypothesis vocabulary live in the non-default WIP lib
(Surface.lean:34-947, RealisabilitySpec.lean:206) despite being sorry-free. A reviewer reading
the nominal trusted surface cannot state what the project claims, and the trusted-vocabulary
defs sit in a file whose header advertises tracked sorry debt. **Fix**: misplacement #1 — hoist
the statement-level defs; leave only sorry'd theorems in WIP. (The skeleton itself is WIP; the
smell is the *layering of its sorry-free vocabulary*.)
...
**5.3 The flagship's hypothesis envelope is known-incomplete while billed as final.** Two static
facts the proof plan itself says are needed are absent from `WellLowered` and every flagship
hypothesis: (a) the R6 size bound `(flatBytes prog).length ≤ 2^32` (Machinery.lean:1513 `hsize`;
the flagship's own blocker comment concedes it "has no producer from hwl",
RealisabilitySpec.lean:235-237); (b) `RevalidatesPerBlock` (Surface.lean:312), the static half
R0b rests on, not a WellLowered field (:401-451). Since RealisabilitySpec.lean:197-205 bills the
R11 ledger as "the honest surface, nothing else", this is a statement-level gap, not pending
proof work. **Fix**: add `size` and `revalidates` fields to WellLowered *now* so the reviewed
statement is the one that will land — both are in this track's own edit surface.
...
"drift-proof" register can silently diverge from the real flagship hypothesis (the noErase
textual twin). Also: AcyclicWellFormed re-states seven WellFormedLowered fields
character-for-character (Acyclic.lean:152-198 vs LowerConforms.lean:150-211) — factor a shared
`LoweredBounds`.

**5.8 Expr.slot pollutes the source IR grammar.** IR.lean:94 is documented "never produced by a
source program, never evaluated" (evalExpr ⇒ none, Semantics.lean:147), yet sits in the trusted
grammar; the flagship carries `WellLowered.noSlotSource` (Surface.lean:450-451) to exclude the
degenerate programs it enables; every IR-level lemma carries a dead `.slot` arm; and the
Loc/Alloc layer built to name the policy (Lowering.lean:94-113) immediately collapses back into
Expr via `Loc.toDef`. **Fix**: make materialiseExpr/MatDec/chargeOf recurse on
`Tmp → Option Loc` (or an internal lowered-expr type), deleting the marker and the noSlotSource
obligation — the folklore "no source program writes .slot" becomes a type-level fact.
...
recovered only via WellLowered; consider `Option ByteArray`); `sim_call`/`sim_create` pure
re-exports of exp003 constructors (Frame/Match.lean:481/:543); four hand-maintained lockstep
recursions materialiseExpr/chargeOf/MatDec/MatFueled (deliberate and documented; one annotated
emission function would make lockstep by construction); identity/alias theorems
...
defsOf reroutes every non-recomputable temp to `Expr.slot (slotOf t)` (Lowering.lean:262-272);
...
Decided for plumbing reuse (materialiseExpr consumes `Tmp → Option Expr`). **Verdict:
should-change** (smell 5.8): it collapses the Loc/Alloc abstraction built to name the policy,
plants a compiler-internal constructor in the trusted grammar, and costs a flagship obligation
(noSlotSource) plus dead arms in every IR-level lemma. Recurse on Loc directly.
...
`Acyclic` survives only as *def-graph* rank for recompute fuel (Acyclic.lean:82), a different
concept. **Verdict: sound.** The dynamic measure is strictly more general and the code
correctly separates the two acyclicity notions (readers initially conflated them; adjudicated:
distinct, both live — Acyclic's consumer is the R9 witness, Witness.lean:363-435).
...
engine bricks). The one statement-level gap is smell 5.3 (WellLowered missing two fields).
...
**D11. WellLowered as the single static premise (decidable-in-principle, R9 checker target).**
**Verdict: sound in shape, incomplete in content** (smell 5.3): add the size and revalidates
fields before further review cycles, so the reviewed ledger is the landing ledger.

**D12. `lower` total, garbage-on-ill-formed-input.** materialiseExpr emits `[]` on fuel
exhaustion and `PUSH32 0` on undefined tmps (Lowering.lean:145/:149); soundness recovered via
WellLowered. **Verdict: defensible convenience; revisit toward `Option ByteArray`** when the
grammar work (D4) is done — garbage-in should produce `none`, not runnable-but-wrong bytes.
...
- Spec/Conformance.lean stub: confirmed verbatim (deletion notice, forwarding pointer with the
  stale path missing the `Realisability/` segment).
