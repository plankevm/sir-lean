...
**Scope**: `LirLean/Realisability/` in full, `LirLean/Drive/`, plus the call-channel
companions [`Call.lean`](../../../LirLean/Call.lean),
[`CallRealises.lean`](../../../LirLean/CallRealises.lean),
...
[`lower_conforms`](../../../LirLean/Realisability/RealisabilitySpec.lean#L251) — run the
...
([`RecorderCoupled`](../../../LirLean/Realisability/Surface.lean#L234)). The folder is
...
[`runFrom_of_driveCorrLog`](../../../LirLean/Realisability/Producer.lean#L1442) and the R6
...
([by design](../../../LirLean/Realisability/RealisabilitySpec.lean#L424)).
...
[`Drive/Headline.lean` header](../../../LirLean/Drive/Headline.lean#L17). The surviving
in-tree driver
[`lower_conforms_cyclic'`](../../../LirLean/Drive/DriveSim.lean#L661) is itself honest
about its own restriction: its `RunDefinable` premise is
[unsatisfiable for any program using `.call`/`.gas`](../../../LirLean/Drive/DriveSim.lean#L666)
...
[`TermTies'`](../../../LirLean/Realisability/Surface.lean#L457) are then **built** from
the run (`R10`), never supplied. Eight numbered "vacuity lessons" shaping the statements are
recorded in the
[`RealisabilitySpec` header](../../../LirLean/Realisability/RealisabilitySpec.lean#L17)
...
coupled producer
[`runFrom_of_driveCorrLog`](../../../LirLean/Realisability/Producer.lean#L1442) — and
...
All three live in
[`RealisabilitySpec.lean`](../../../LirLean/Realisability/RealisabilitySpec.lean), the
...
### 2.1 [`lower_conforms`](../../../LirLean/Realisability/RealisabilitySpec.lean#L251) (R11, plain)
...
| `hself` | decidable entry fact | the recipient (self) account exists in the pre-call world; seeds [`SelfPresent`](../../../LirLean/Drive/SelfPresent.lean#L353) |
...
### 2.2 [`lower_conforms_exact`](../../../LirLean/Realisability/RealisabilitySpec.lean#L302) (R11-all)
...
([sorry at #L333](../../../LirLean/Realisability/RealisabilitySpec.lean#L333)) demands an
**exact producer**; the shell comments (and the plan's chunk-7 no-go) forbid deriving
exactness post-hoc from the plain producer.

### 2.3 [`lower_conforms_gasfree`](../../../LirLean/Realisability/RealisabilitySpec.lean#L341) (co-flagship)
...
[`realisedGas_nil_of_noGasReads`](../../../LirLean/Realisability/RealisabilitySpec.lean#L376)
...
([lesson 4](../../../LirLean/Realisability/RealisabilitySpec.lean#L34)), `SstoreRealises`
is free-∀ unsatisfiable ([lesson 3](../../../LirLean/Realisability/RealisabilitySpec.lean#L28)),
and `StepScoped`'s live-scope clause is refuted by the witness program's own loop
([lesson 8](../../../LirLean/Realisability/RealisabilitySpec.lean#L67)). All three were
replaced (`RunDefinableG`, point-wise R4, `StepScopedS`/`invalStep`). The machine-checked
anti-vacuity anchors:

- [`exProg`](../../../LirLean/Realisability/Witness.lean#L38) — the witness program
  ...
  **loop**), with the full static bundle proved concretely:
  [`irWellFormed_exProg`](../../../LirLean/Realisability/Witness.lean#L548),
  [`codeFits_exProg`](../../../LirLean/Realisability/Witness.lean#L560),
  [`stackFits_exProg`](../../../LirLean/Realisability/Witness.lean#L565),
  [`wellLowered_exProg`](../../../LirLean/Realisability/Witness.lean#L590), and the
  R9 checker existential
  [`wellLowered_check_exists`](../../../LirLean/Realisability/Witness.lean#L608)
  (sound checker + accepts the witness). Witness.lean is entirely sorry-free.
- Still debt: the **runtime** half of non-vacuity.
  [`exProg_satisfies_hypotheses`](../../../LirLean/Realisability/RealisabilitySpec.lean#L388)
  (R12a, `sorry`) must exhibit concrete `params`/`log` making `hrun`+`hclean`+`hseams` true;
  [`exProg_nonvacuity`](../../../LirLean/Realisability/RealisabilitySpec.lean#L401)
...
### 3.1 [`RecorderCoupled`](../../../LirLean/Realisability/Surface.lean#L234) — the load-bearing invariant
...
### 3.2 [`DriveCorrLog`](../../../LirLean/Realisability/Surface.lean#L270) — the coupled walk carrier
...
entry by [`driveCorrLog_entry`](../../../LirLean/Realisability/Producer.lean#L168)
...
### 3.3 [`StmtTies'`](../../../LirLean/Realisability/Surface.lean#L351) / [`TermTies'`](../../../LirLean/Realisability/Surface.lean#L457) — run-derived per-block ties
...
[Surface.lean#L336](../../../LirLean/Realisability/Surface.lean#L336)). The gas arm is
...
DERIVED: [`termTies'_of_runWithLog`](../../../LirLean/Realisability/Producer.lean#L2642)
(R10b) is **closed** via
[`termTies'_of_walk`](../../../LirLean/Realisability/Machinery.lean#L503);
[`stmtTies'_of_runWithLog`](../../../LirLean/Realisability/Producer.lean#L2488)
(R10a) is still `sorry` (§5).

### 3.4 [`CallRealisesS`](../../../LirLean/Realisability/Surface.lean#L78) — the shadowing-aware CALL kernel
...
[lesson 7](../../../LirLean/Realisability/RealisabilitySpec.lean#L58)).

### 3.5 Producer vocabulary — [`Producer.lean` §0](../../../LirLean/Realisability/Producer.lean#L60) (all real)
...
— [`StreamsAligned`](../../../LirLean/Realisability/Producer.lean#L74): at every coupled
...
[`RunFromCoupled`](../../../LirLean/Realisability/Producer.lean#L83) packages "some IR
observable whose world/result equal the bytecode terminal's `observe`" + the IR `RunFrom`;
[`DriveLogStep`](../../../LirLean/Realisability/Producer.lean#L97) is the coupled
per-block step (halt, or advance to a strictly-smaller-`totalGas` coupled successor);
[`CoupledAdvance`](../../../LirLean/Realisability/Producer.lean#L114) is one coupled
...
— [`runFrom_of_driveCorrLog`](../../../LirLean/Realisability/Producer.lean#L1442)
(`sorry`; the blocker). Its `hsize` is derived from `hcodeFits` at the flagship call sites
(chunk-0 wiring, landed); `WellLowered` is the
[internal adapter](../../../LirLean/Realisability/Surface.lean#L151) rebuilt from the
...
[the two documented reasons](../../../LirLean/Realisability/Producer.lean#L15): the
...
`rg -n "\bsorry\b" LirLean/Realisability` on current source: **16 proof-position
...
|---|---|---|---|---|
| 1 | [`atReachableBoundaryVJ_step`](../../../LirLean/Realisability/Machinery.lean#L1372) | [#L1390](../../../LirLean/Realisability/Machinery.lean#L1390), [#L1398](../../../LirLean/Realisability/Machinery.lean#L1398) | R6 bricks **B-pc** (successor pc = sequential or `validJumps` member) and **B-inrange** (sequential successor stays in range) | 1 |
| 2 | [`atReachableBoundaryVJ_call`](../../../LirLean/Realisability/Machinery.lean#L1416) | [#L1431](../../../LirLean/Realisability/Machinery.lean#L1431) | R6 **B-inrange** (CALL instance: `b + 1 < length`) | 1 |
| 3 | [`atReachableBoundaryVJ_create`](../../../LirLean/Realisability/Machinery.lean#L1462) | [#L1464](../../../LirLean/Realisability/Machinery.lean#L1464) | R6 **CREATE resume edge** — no longer vacuous since the lowering emits create bytes; also needs an `hsize` signature fix (plan chunk 1) | 1 (+3) |
| 4 | [`callRealises_of_recorded`](../../../LirLean/Realisability/Machinery.lean#L392) | [#L413](../../../LirLean/Realisability/Machinery.lean#L413) | R3 — Piece A (record extraction from the coupling) is **landed** ([`recorderCoupled_call_extract`](../../../LirLean/Realisability/Machinery.lean#L1929)); the sorry is **Piece B**: no in-tree producer builds the CALL arg-push `Runs fr0 callFr` (~200-line materialise driver) | 2 |
| 5 | [`simStmt_coupled_gas`](../../../LirLean/Realisability/Producer.lean#L453) | [#L467](../../../LirLean/Realisability/Producer.lean#L467) | coupled GAS arm (fires R1 + the gas sim brick + R7b) | 4 |
| 6 | [`simStmt_coupled_sload`](../../../LirLean/Realisability/Producer.lean#L474) | [#L489](../../../LirLean/Realisability/Producer.lean#L489) | coupled SLOAD arm (fires tie arm 2 + R7c) | 4 |
| 7 | [`simStmt_coupled_call`](../../../LirLean/Realisability/Producer.lean#L1279) | [#L1296](../../../LirLean/Realisability/Producer.lean#L1296) | coupled CALL arm — gated on R3 Piece B | 4 (⇐ 2) |
| 8 | [`stmtTies'_of_runWithLog`](../../../LirLean/Realisability/Producer.lean#L2488) | [#L2498](../../../LirLean/Realisability/Producer.lean#L2498) | R10a — build the statement ties from the run | 3→5 seam (order-of-work item 3) |
| 9 | [`simStmts_coupled_block`](../../../LirLean/Realisability/Producer.lean#L1327) | [#L1344](../../../LirLean/Realisability/Producer.lean#L1344) | P3a coupled block walk | 5 |
| 10 | [`driveLogStep_of_block`](../../../LirLean/Realisability/Producer.lean#L1359) | [#L1374](../../../LirLean/Realisability/Producer.lean#L1374) | P3b coupled per-block step | 5 |
| 11 | [`runFrom_of_driveCorrLog_rec`](../../../LirLean/Realisability/Producer.lean#L1385) | [#L1397](../../../LirLean/Realisability/Producer.lean#L1397) | P4 coupled `totalGas` recursion | 5 |
| 12 | [`runFrom_of_driveCorrLog`](../../../LirLean/Realisability/Producer.lean#L1442) | [#L1461](../../../LirLean/Realisability/Producer.lean#L1461) | the packaged producer (top-level assembly) | 6 |
| 13 | [`lower_conforms_exact`](../../../LirLean/Realisability/RealisabilitySpec.lean#L302) | [#L333](../../../LirLean/Realisability/RealisabilitySpec.lean#L333) | the exact producer `obtain` | 7 |
| 14 | [`realisedGas_nil_of_noGasReads`](../../../LirLean/Realisability/RealisabilitySpec.lean#L376) | [#L381](../../../LirLean/Realisability/RealisabilitySpec.lean#L381) | gasfree companion (needs R6 walk) | 8 |
| 15 | [`exProg_satisfies_hypotheses`](../../../LirLean/Realisability/RealisabilitySpec.lean#L388) | [#L396](../../../LirLean/Realisability/RealisabilitySpec.lean#L396) | R12a concrete-params witness grind | order-of-work item 6 |
...
[`boundaryWalk_of_wl`](../../../LirLean/Realisability/Producer.lean#L1403) (closed), the
**B-call** brick
...
consumed at [Machinery.lean#L1428](../../../LirLean/Realisability/Machinery.lean#L1428),
...
[#L1408](../../../LirLean/Realisability/Machinery.lean#L1408) and
[#L1506](../../../LirLean/Realisability/Machinery.lean#L1506) still list B-call among the
...
- [`RecorderCoupled`](../../../LirLean/Realisability/Surface.lean#L234) has **no create
  suffix** — its `restart` field pins the future's create stream to the whole `log.creates`;
- [`StreamsAligned`](../../../LirLean/Realisability/Producer.lean#L74) pins
  `D = createStreamOf log.creates self` at **every** boundary;
- [`StmtTies'`](../../../LirLean/Realisability/Surface.lean#L351) has five arms and **no
...
[`realisedCreate_cons`](../../../LirLean/RecorderLemmas.lean#L164),
[`createRealises_bridge`](../../../LirLean/CallRealises.lean#L118),
[`createPreservesSelf_modGuards`](../../../LirLean/Drive/CallPreservesSelf.lean#L300),
and the create arm of
[`defsSoundS_preserved_step`](../../../LirLean/Realisability/Machinery.lean#L209).
...
- **R0** — the reshaped tie *statements* `StmtTies'`/`TermTies'` and the whole §1–§4
  vocabulary of [`Surface.lean`](../../../LirLean/Realisability/Surface.lean) (real defs,
  no sorry).
- **R0b** — [`defsSoundS_preserved_step`](../../../LirLean/Realisability/Machinery.lean#L88):
...
- **R1** — [`gas_suffix_head_realised`](../../../LirLean/Realisability/Machinery.lean#L1650):
  the historically-riskiest trace↔recorder gas bridge, **closed** (decode from `Corr` +
  clean-halt extraction + R7b head pinning).
- **R2** — [`haltNonException_of_cleanLog`](../../../LirLean/Realisability/Machinery.lean#L268):
  `log.clean` ⇒ every halting terminal is non-exception (drive adequacy + halting-terminal
  uniqueness via [`runs_halt_eq`](../../../LirLean/Realisability/Machinery.lean#L248)).
- **R4** — [`sstoreRealises_at_frame`](../../../LirLean/Realisability/Machinery.lean#L424):
...
- **R5/R10b** — [`termTies'_of_walk`](../../../LirLean/Realisability/Machinery.lean#L503)
  and [`termTies'_of_runWithLog`](../../../LirLean/Realisability/Producer.lean#L2642):
  all four terminator ties derived (the one `maxRecDepth 8192` bump in the folder sits here).
- **R6, partial** — entry seed
  [`atReachableBoundaryVJ_entry`](../../../LirLean/Realisability/Machinery.lean#L1352),
  the strengthened invariant
  [`AtReachableBoundaryVJ`](../../../LirLean/Realisability/Machinery.lean#L1345), the
  `Runs`-induction combinator
  [`atReachableBoundaryVJ_of_runs`](../../../LirLean/Realisability/Machinery.lean#L1472),
  the taken-jump arm, and the B-call brick; the top-level
  [`runs_atReachableBoundary`](../../../LirLean/Realisability/Machinery.lean#L1519) is
  real assembly over the (partially sorry'd) edges. The counterexample
  [`not_runs_atReachableBoundary`](../../../LirLean/Realisability/Machinery.lean#L1279)
...
- **R7a–e′** — the full recorder-coupling edge algebra:
  [`recorderCoupled_entry`](../../../LirLean/Realisability/Machinery.lean#L1537),
  [`recorderCoupled_step_gas`](../../../LirLean/Realisability/Machinery.lean#L1556),
  [`recorderCoupled_sload`](../../../LirLean/Realisability/Machinery.lean#L1681),
  [`recorderCoupled_step_other`](../../../LirLean/Realisability/Machinery.lean#L1719),
  [`recorderCoupled_call`](../../../LirLean/Realisability/Machinery.lean#L1839) (one
  `CallRecord`, no gas/sload, consumed per returning CALL),
  [`recorderCoupled_call_extract`](../../../LirLean/Realisability/Machinery.lean#L1929)
  (R3 Piece A: produces the `CallReturns` witness and the record identity from the coupling)
  and [`recorderCoupled_stepsTo_other`](../../../LirLean/Realisability/Machinery.lean#L2007).
  Method: fuel destruction + the accumulator homomorphism
  [`driveLog_acc_hom`](../../../LirLean/Realisability/Machinery.lean#L1142).
- **R8** — [`present_of_closed`](../../../LirLean/Realisability/Machinery.lean#L2021).
- **R9** — the witness stack and singleton checker (§2.4; Witness.lean sorry-free,
  including the loop-staleness machine-check
  [`not_defsSound_stale`](../../../LirLean/Realisability/Witness.lean#L236) that forced
  R0b).
...
- [`streamsAligned_entry`](../../../LirLean/Realisability/Producer.lean#L137) /
  [`driveCorrLog_entry`](../../../LirLean/Realisability/Producer.lean#L168) — the
  induction base, including the entry-storage reconciliation against `codeAccounts`.
- [`simStmt_coupled_assignPure`](../../../LirLean/Realisability/Producer.lean#L398) — the
  plain-assign coupled arm, fully closed (fires tie arm 1; frame unchanged, `Runs.refl`).
- [`recorderCoupled_matRunsC`](../../../LirLean/Realisability/Producer.lean#L510) — the
...
- [`sim_sstore_stmt'`](../../../LirLean/Realisability/Producer.lean#L1036) and
  [`simStmt_coupled_sstore`](../../../LirLean/Realisability/Producer.lean#L1199) — the
  SSTORE coupled arm, fully closed including zero-writes/slot-clears (the former
  nonzero-write scope seam is gone).
- [`boundaryWalk_of_wl`](../../../LirLean/Realisability/Producer.lean#L1403) (P5) and
  [`createResolves_reachable`](../../../LirLean/Realisability/Producer.lean#L1423) (P6).
...
1. The [plan checkpoint](../../planning/r11-plan-2026-07-08.md) says a box run "closed B-pc
   ...
   ([B-pc](../../../LirLean/Realisability/Machinery.lean#L1390),
   [CALL in-range](../../../LirLean/Realisability/Machinery.lean#L1431)) are still open
...
2. Stale R6 docstrings: [`atReachableBoundaryVJ_call`](../../../LirLean/Realisability/Machinery.lean#L1408)
   and [`runs_atReachableBoundary`](../../../LirLean/Realisability/Machinery.lean#L1506)
   still list B-call among the sorry'd bricks though it is discharged at
   [#L1428](../../../LirLean/Realisability/Machinery.lean#L1428). The plan's cleanup gate
   ("compress the new per-op support and stale R6 prose") already covers this.
3. Minor line-drift in comments: e.g.
   [Producer.lean#L1276](../../../LirLean/Realisability/Producer.lean#L1276) cites
   `callRealises_of_recorded` at "Machinery.lean:405" (now
   [#L392](../../../LirLean/Realisability/Machinery.lean#L392)).
...
- Two `maxRecDepth` bumps: `8192` on
  [`termTies'_of_walk`](../../../LirLean/Realisability/Machinery.lean#L466) (under the
  headline chain — a symptom of the very large `TermTies'` branch-arm term, not of `decide`
  abuse) and `8000` on the concrete
  [`codeFits_exProg`](../../../LirLean/Realisability/Witness.lean#L557)/`stackFits_exProg`
...
- The WIP lib intentionally has **no** `#print axioms` guards
  ([documented](../../../LirLean/Realisability/RealisabilitySpec.lean#L424)); guards
  ...
  the proof bodies).
- [`Classical.choose` in `driveLogStep_of_block`'s statement](../../../LirLean/Realisability/Producer.lean#L1367)
...
already-proved [`runFrom_of_driveCorr`](../../../LirLean/Drive/DriveSim.lean#L580).
