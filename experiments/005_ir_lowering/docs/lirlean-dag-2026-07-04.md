...
| CallRealises | 146 | **terminal + incremental.** `callRealises_bridge` and `createRealises_bridge` connect the recorder entries to lowered CALL/CREATE observables. The live `evmCallEntry` definition is in `Spec/Recorder`. |
| Decode/Modellable | 483 | **terminal.** `modellable_of_runs` applied RS:1255; residual seams `AtReachableBoundary` (RS:1245) + `CallsCode` (Seams:81); `notCreate_of_atReachableBoundary` (`:426`) wired RS:1255/3677. The no-CREATE combinator run is unavoidable structural infra. |
