...
| SmallStep.lean | 131 | IR small-step operational state (`IRState`, `evalExpr`, halt) — the Match-layer IR | `IRState` (`:49`), `evalExpr` (`:89`), `IRHalt`/`IRConf` (`:60`/`:69`), `IRState.applyCall`-support | **both** (via Match) | load-bearing | NB: this `IRState` is distinct from the flagship's `Lir.IRState`; two IR-state defs coexist (consolidation lever, §3) |
