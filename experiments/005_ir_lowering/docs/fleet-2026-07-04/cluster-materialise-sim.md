...
cyclic flagship (`V2/RealisabilitySpec.lean`, the WIP sorry-carrier).
...
* **~3347 LOC genuinely reused** by the cyclic flagship via two entry points:
  `V2.Drive.SelfPresent → MaterialiseRuns` and `V2.CallRealises → Match` (plus
  `V2.Call → DefsSound`). Transitive closure of those pulls in
...
| SmallStep.lean | 131 | IR small-step operational state (`IRState`, `evalExpr`, halt) — the Match-layer IR | `IRState` (`:49`), `evalExpr` (`:89`), `IRHalt`/`IRConf` (`:60`/`:69`), `IRState.applyCall`-support | **both** (via Match) | load-bearing | NB: this `IRState` is distinct from the flagship's `V2.IRState`; two IR-state defs coexist (consolidation lever, §3) |
| Match.lean | 595 | `Match` invariant + per-construct frame-local sim bricks (`sim_imm/add/lt/sload/sstore/call`) | `Match` (`:125`), `sim_imm`/`sim_add`/`sim_sload`/`sim_sstore`/`sim_call` (`:150`+), `call_reflects_lowered` (`:519`), `pcOf` (`:66`) | **both** (cyclic via `CallRealises`) | load-bearing | the sim-brick library both stacks lean on |
...
| DefsSound.lean | 579 | B3: `WellFormed` + `DefsSound` recompute-on-use soundness | `DefsSound` (`:198`), `WellFormed` (`:132`), `defsSound_preserved_*` (`:290`+), `usesInExpr`/`useCount` (`:51`/`:78`), `StepScoped` (`:514`) | **both** (via MaterialiseRuns + V2.Call) | load-bearing | keep; the recompute-soundness foundation both stacks need |
...
```
 ...
 MatDecLower   MaterialiseCleanHalt   StashTail
   ...
   └──────────────┴────────► LowerDecode ──► LowerConforms ──► Acyclic ──► [FLAGSHIP]
                                                    ▲
                                     V2.DriveSim ───┘ (also reaches LowerConforms!)

  CYCLIC REUSE ENTRY POINTS (bypass LowerConforms):
    V2.Drive.SelfPresent ──► MaterialiseRuns
    V2.CallRealises      ──► Match
    V2.Call              ──► DefsSound
```
...
* **Foundation (reused, survives):** SmallStep, Call, StorageErase, DefsSound, Match,
  ...
  CallRealises / V2.Call, independently of `LowerConforms`.
...
**and** `RealisabilitySpec → V2.Drive.Headline → V2.DriveSim → LowerConforms`
(`V2/DriveSim.lean:1`). Deleting `Acyclic` alone does **not** orphan the sim stack — DriveSim
still pulls `LowerConforms` and therefore the whole Layer-C/D/E subtree. Realising the
3627-line win requires **first rewiring `V2.DriveSim` off `LowerConforms`** (out of cluster,
...
`SmallStep.IRState` (`:49`, the Match-layer IR) and the flagship's `V2.IRState` (V2/Machine)
