...
| `messageCall_runs` | `003_bytecode_layer/BytecodeLayer/Hoare/CallSequence.lean:132` | boundary bridge into observables |
| `runWithLog_messageCall` | `LirLean/V2/RunLog.lean` (line not pinned in maps) | pins `messageCall = .ok` from the recording run |
...
| `totalGas` | `V2/Measure.lean` (line `:0` in map) | structural drive measure (frame `gasAvailable`, strictly descends/block) |
...
| `RunDefinable` | `V2/IRRun.lean:257` | scope-premise; **benign definability, NOT acyclicity** |
...
| `runWithLog` / `driveLog` / `RunLog` / `recordCall` / `CallRecord` | `V2/RunLog.lean:219/156/82/145/68` | lowering (recording interpreter) |
...
All in `EVMLean/Evm/…` (leanevm base) and `003_bytecode_layer/BytecodeLayer/…`. Not proven in this project — the trust root (see §5).
...
| `BytecodeLayer.Hoare.Runs` / `StepsTo` / `CallReturns` / `EntersAsCode` | `003_bytecode_layer/BytecodeLayer/Hoare.lean:114/52/91`, `Semantics/System.lean:237` |
| `Runs.linear_to_halt` / `Runs.gasAvailable_le` | `003_bytecode_layer/BytecodeLayer/Hoare.lean:240`, `Hoare/GasMonotone.lean:251` |
| `CallResult.observe` | `003_bytecode_layer/BytecodeLayer/Observables.lean:36` |
| IR semantics cores: `RunFrom` / `IRRun` / `RunStmts` / `EvalStmt` / `stmtPost` / `stmtsPost` | `V2/Machine.lean:228/274`, `V2/IRRun.lean:112/…`, `V2/Machine.lean:166`, `V2/IRRun.lean:69/112` |

*(`totalGas` was cited `:0`; its file is `V2/Measure.lean`. `runWithLog_messageCall`, `sim_stmts_block`, and a few `:0`-annotated cross-cluster leaves were not opened to an exact line in the maps — flagged rather than invented.)*
...
- The realisability discharge at the concrete boundary is **rfl-clean**: `call_reflects_lowered` (`Match.lean:436`), `callRealises_bridge` (`V2/CallRealises.lean:90`), `evmCallOracle_successWord_eq_x` (`Call.lean:129`), `realisedCall_eq_evmV2` (`V2/RunLog.lean:280`) — so the IR's call effect *is* the lowered bytecode's by construction, never assumed.

**Recommendation:** keep `V2.CallOracle` + `CallReturns` as the oracle; the surrounding frame-threading is realised (dischargeable), so a tightened `CallRealises` could shed the plumbing conjuncts.
...
- `V2.CFGAcyclic` (`V2/IRRun.lean:225`) via `blockRank : Label → ℕ` makes the `RunFrom` recursion well-founded (`TermRankLt` `:205`, `Term.succs` `:212`, decreasing + succ_present).
- `runFrom_exists` (`V2/IRRun.lean:291`, strong induction on `blockRank`) → `irRun_exists` (`:340`) **CONSTRUCTS** the IR run instead of assuming it.
