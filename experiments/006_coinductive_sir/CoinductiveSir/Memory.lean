...
def readByte? (a : Allocation) (address : Nat) : Option UInt8 :=
  if h : a.start ≤ address ∧ address < a.endExclusive then
    a.bytes[address - a.start]'(by grind [Allocation.start, Allocation.size, Allocation.endExclusive])
  else
    none

def writeByte (a : Allocation) (address : Nat) (value : UInt8) : Allocation :=
  if h : a.start ≤ address ∧ address < a.endExclusive then
    let bytes' := a.bytes.set
      (address - a.start)
      value
      (by grind [Allocation.start, Allocation.size, Allocation.endExclusive])
    { a with bytes := bytes' }
  else
    a
...
def writeBytes (m : MemoryState) (offset : U256) (bytes : ByteArray) : MemoryState :=
