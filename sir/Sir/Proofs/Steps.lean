...
theorem fires_of
    ...
    localOperandFrame.Fires policy ctx operation src dst env globals trace env' globals' :=
...
theorem firesHalt_false
    ...
    (h : localOperandFrame.FiresHalt policy ctx operation src env globals trace globals') :
    False := by
  cases h with
...
theorem step_statement
    ...
    (hfires : (decodeSirStatement statement).Fires policy ctx state.locals state.globals
      trace locals' globals') :
    GenericStep localOperandFrame (sirDecoder program) policy ctx state.toGenericState trace
...
theorem step_assign
    ...
    (hfires : (decodeExpression result expr).Fires policy ctx state.locals state.globals
      trace locals' globals') :
    GenericStep localOperandFrame (sirDecoder program) policy ctx state.toGenericState trace
...
theorem step_sstore
    ...
    (hfires : (decodeSirStatement (.sstore key value)).Fires policy ctx
...
theorem step_gas
    ...
    (hfires : (decodeSirStatement (.gas result)).Fires policy ctx
      state.locals state.globals trace locals' globals') :
    GenericStep localOperandFrame (sirDecoder program) policy ctx state.toGenericState trace
      ({ globals := globals', locals := locals', control := next } : MachineState).toGenericState :=
  step_statement hdecode hfires
...
theorem step_call
    ...
    (hfires : (decodeSirStatement (.call call)).Fires policy ctx
      state.locals state.globals trace locals' globals') :
    GenericStep localOperandFrame (sirDecoder program) policy ctx state.toGenericState trace
      ({ globals := globals', locals := locals', control := next } : MachineState).toGenericState :=
  step_statement hdecode hfires
...
theorem step_mallocUninit
    ...
    (hfires : (decodeSirStatement (.mallocUninit result size)).Fires policy ctx
      state.locals state.globals trace locals' globals') :
    GenericStep localOperandFrame (sirDecoder program) policy ctx state.toGenericState trace
      ({ globals := globals', locals := locals', control := next } : MachineState).toGenericState :=
  step_statement hdecode hfires
...
theorem step_mstore32
    ...
    (hfires : (decodeSirStatement (.mstore32 offset value)).Fires policy ctx
      state.locals state.globals trace locals' globals') :
    GenericStep localOperandFrame (sirDecoder program) policy ctx state.toGenericState trace
      ({ globals := globals', locals := locals', control := next } : MachineState).toGenericState :=
  step_statement hdecode hfires
...
theorem step_mload32
    ...
    (hfires : (decodeSirStatement (.mload32 result offset)).Fires policy ctx
      state.locals state.globals trace locals' globals') :
    GenericStep localOperandFrame (sirDecoder program) policy ctx state.toGenericState trace
      ({ globals := globals', locals := locals', control := next } : MachineState).toGenericState :=
  step_statement hdecode hfires
...
theorem step_terminator
    ...
    GenericStep localOperandFrame (sirDecoder program) policy ctx state.toGenericState [] state'.toGenericState := by
...
theorem step_icall
    ...
    (hcallee : GenericFunctionEvaluation localOperandFrame (sirDecoder program) policy ctx callee
...
theorem EvalFn.returned
    ...
    (hrun : GenericSteps localOperandFrame (sirDecoder program) policy ctx
      initial.toGenericState trace exit.toGenericState)
    (hreturn : exit.control = .returned results) :
    GenericFunctionEvaluation localOperandFrame (sirDecoder program) policy ctx function globals args
...
@[elab_as_elim]
theorem Steps.inductionOn {program : Program} {policy : Generic.MemoryPolicy} {ctx : CallContext}
    {motive : (state : MachineState) → (trace : Trace) → (final : MachineState) →
      GenericSteps localOperandFrame (sirDecoder program) policy ctx
    ...
    (tail : ∀ {state middle final : MachineState} {trace₁ trace₂ : Trace}
      (start : GenericSteps localOperandFrame (sirDecoder program) policy ctx
        state.toGenericState trace₁ middle.toGenericState)
      (next : GenericStep localOperandFrame (sirDecoder program) policy ctx
    ...
    (h : GenericSteps localOperandFrame (sirDecoder program) policy ctx
...
def Stuck (program : Program) (policy : Generic.MemoryPolicy) (ctx : CallContext)
    (state : MachineState) : Prop :=
  Generic.Stuck (sirDecoder program) policy ctx state.toGenericState

theorem stuck_of_returned
    {state : MachineState} {results : Array Word}
    (hcontrol : state.control = .returned results) :
    Stuck program policy ctx state :=
...
theorem Steps.eq_of_stuck
    ...
    (hstuck : Stuck program policy ctx state) : final = state ∧ trace = [] := by
...
theorem stepDialogue_all
    ...
    {state final : MachineState} {trace : Trace}
    (h : GenericStep localOperandFrame (sirDecoder program) policy ctx
      state.toGenericState trace final.toGenericState) :
    ∀ (trace₂ : Trace) (final₂ : MachineState),
      GenericStep localOperandFrame (sirDecoder program) policy ctx
        state.toGenericState trace₂ final₂.toGenericState →
      (trace = trace₂ ∧ final = final₂) ∨
        Trace.QueryDivergence trace trace₂ := by
  intro trace₂ final₂ h₂
  rcases Generic.stepDialogue_all
      (Generic.allocationDeterministic_noMalloc halloc)
...
theorem runDialogue_all
    ...
    ∀ (trace₂ : Trace) (final₂ : MachineState),
      GenericSteps localOperandFrame (sirDecoder program) policy ctx
        state.toGenericState trace₂ final₂.toGenericState →
      (∃ suffix, GenericSteps localOperandFrame (sirDecoder program) policy ctx
...
theorem fnDialogue_all
    ...
    (h : GenericFunctionEvaluation localOperandFrame (sirDecoder program) policy ctx
      function globals args trace globals' outcome) :
    ∀ trace₂ globals₂ outcome₂,
      GenericFunctionEvaluation localOperandFrame (sirDecoder program) policy ctx
        function globals args trace₂ globals₂ outcome₂ →
...
private theorem genStep_preserves_function
    ...
    (h : GenericStep localOperandFrame (sirDecoder program) policy ctx state trace final)
    (hcontrol : state.control = .running cursor) :
