...
theorem StackSchedule.source_step_trace_empty
  ...
  | operation hdecode fires =>
      obtain ⟨notGas, notCall⟩ := certificate.source_decode_primitive_pure accepted hdecode
  ...
  | internalCall hdecode fetch evaluation resume =>
      exact (certificate.source_decode_icall_false accepted hdecode).elim
  | control controlStep =>
...
theorem StackSchedule.target_step_trace_empty
  ...
  cases step with
  | operation hdecode fires =>
      obtain ⟨notGas, notCall⟩ := certificate.target_decode_primitive_pure accepted hdecode
      exact Stack.frame.fires_trace_nil notGas notCall fires
  | operationHalted hdecode fires =>
      exact (Machine.OperandFrame.firesHalt_false _ fires).elim
  | internalCall hdecode fetch evaluation resume =>
      exact (certificate.target_decode_icall_false accepted hdecode).elim
...
theorem StackSchedule.target_step_not_returned
  ...
  | internalCall hdecode fetch evaluation resume =>
      exact certificate.target_decode_icall_false accepted hdecode
  | control controlStep =>
      simp only [] at returned
      subst returned
      obtain ⟨cursor, block, controlEq, blockAt, terminatorEq⟩ :=
        stackControl_returned_inv controlStep
      exact certificate.stack_program_terminator_not_iret accepted cursor block blockAt terminatorEq
