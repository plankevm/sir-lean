...
private theorem toNat_ofNat (n : ℕ) : (Evm.UInt256.ofNat n).toNat = n % 2 ^ 256 := by
  rw [Evm.UInt256.toNat_limbs]
  unfold Evm.UInt256.ofNat
...
theorem isValidNewAlloc_bumpAlloc (m : MemoryState) (size : Nat)
  ...
  have hstart : (m.bumpAlloc size).start = m.watermark % 2 ^ 256 := by
    simp [bumpAlloc, Allocation.start, toNat_ofNat]
...
theorem memoryPolicy_allows_bumpAlloc (m : MemoryState) (size : Nat)
    (hspace : m.watermark + size ≤ Evm.UInt256.size) :
    memoryPolicy.Allows m size (m.bumpAlloc size) :=
  ⟨m.isValidNewAlloc_bumpAlloc size hspace, m.bumpAlloc_size size⟩

theorem bumpPolicy_allows_bumpAlloc (m : MemoryState) (size : Nat)
    (hspace : m.watermark + size ≤ Evm.UInt256.size) :
    bumpPolicy.Allows m size (m.bumpAlloc size) :=
  ⟨rfl, m.isValidNewAlloc_bumpAlloc size hspace⟩

end MemoryState

namespace MemoryPolicy

theorem bump_refines : bumpPolicy.Refines memoryPolicy := by
  intro memory size allocation h
  exact ⟨h.2, h.1 ▸ memory.bumpAlloc_size size⟩
